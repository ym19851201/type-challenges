...
// type Union = string | number | bigint | boolean | null | undefined;
type Join<D extends string, A> = A extends [infer Head, ...infer Rest]
  ? Head extends string
  ? Rest extends []
  ? `${Head}`
  : `${Head}${D}${Join<D, Rest>}`
...
declare function join<D extends string>(delimiter: D): <A extends string[]>(...parts: A) => Join<D, A>;


/* _____________ Test Cases _____________ */
import { Equal, Expect } from '@type-challenges/utils'

const tmp = join('-')('a', 'b', 'c');
// Edge cases
